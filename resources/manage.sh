...
[ -f ${MY_DIR}/proot.cfg ] && source ${MY_DIR}/proot.cfg
...
proot_final_args="${PROOT_ARGS} -w /root -b ${MY_DIR} -b /etc/mtab -b /etc/resolv.conf -b /etc/hostname -b /dev -b /sys -b /proc -r ${MY_DIR} -0"

if [ "$1" = "start-minion" ]; then
        echo "Starting root minion supervisor ..."
        ./proot ${proot_final_args} /usr/bin/supervisord -c /etc/supervisor/supervisord.conf > proot.log 2> proot.err &
fi

if [ "$1" = "stop-minion" ]; then
        echo "Stopping root minion supervisor ..."
        ./proot ${proot_final_args} supervisorctl shutdown
fi

if [ "$1" = "admin" ]; then
        echo "Dropping admin shell ..."
        shift
        ./proot ${proot_final_args} "$@"
fi
